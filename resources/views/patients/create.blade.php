<x-layout>
    <div class="max-w-md mx-auto mt-10 p-6 bg-white shadow-md rounded-md my-8">
        <h1 class="text-2xl font-bold mb-6 text-center">Add Patient</h1>
        <form method="POST" action="{{ route('patients.store') }}" class="space-y-6">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nhs_number" name="nhs_number" value="{{ old('nhs_number') }}" required
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                @error('nhs_number')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div>
                <button type="submit"
                    class="w-full py-2 px-4 bg-indigo-600 text-white font-semibold rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Add Patient</button>
            </div>
        </form>
    </div>
</x-layout>
